...
class ListsTest(TestCase):
	...
	def test_home_page_returns_corret_html(self):
		...
		expect_html = render_to_string('home.djhtml')
...
class ListViewTest(TestCase):
	...
	def test_displays_only_items_for_that_list(self):
		...
		item1 = Item.objects.create(text='other list item 1', list=other_list)
		item2 = Item.objects.create(text='other list item 2', list=other_list)
...
class NewItemTest(TestCase):
	...
	def test_can_save_a_POST_request_to_an_existing_list(self):
		...
		self.client.post('/lists/%d/add_item' % (correct_list.id,),
						 data={'item_text': 'an new item for existing list'})

		self.assertEqual(Item.objects.count(), 1)
		new_item = Item.objects.first()
		self.assertEqual(new_item.text, 'an new item for existing list')
